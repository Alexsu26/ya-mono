...
The configuration system provides a layered approach to managing TUI settings, allowing users to customize behavior while maintaining sensible defaults. Configuration is stored at `~/.xunocli/` following XDG Base Directory specification.
...
xunocli config set general.model "openai:gpt-4o"

# Get a configuration value
xunocli config get general.model

# Reset to defaults
xunocli config reset

# Validate configuration
xunocli config validate

# Copy builtin subagent to user config for customization
xunocli subagent copy search_agent

# List available subagents
xunocli subagent list
```

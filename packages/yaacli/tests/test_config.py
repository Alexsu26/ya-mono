...
def test_mixed_legacy_and_current_model_profiles_are_merged(tmp_path: Path) -> None:
    """Existing fork profiles remain usable alongside new model profiles."""
...
[models."glm-5.2"]
model = "zai@openai-chat:glm-5.2"
model_cfg = "deepseek_v4_1m"
...
"""
    ...
    manager = ConfigManager(config_dir=tmp_path, project_dir=tmp_path / "project")

    config = manager.load()

    assert config.is_configured is True
    assert set(config.model_profiles) == {"glm-5.2", "subs-codex"}
    assert config.model_profiles["glm-5.2"].model == "zai@openai-chat:glm-5.2"


def test_profile_only_config_is_configured() -> None:
    """A selectable profile is sufficient when no legacy default model exists."""
    config = YaacliConfig(
        model_profiles={"fast": ModelProfileConfig(model="openai-responses:gpt-5-mini")},
    )
...
def test_persist_display_theme_preserves_existing_global_config(
    config_manager: ConfigManager,
    temp_config_dir: Path,
) -> None:
    config_file = temp_config_dir / "config.toml"
    ...
    result = config_manager.persist_display_theme("cappuccino")

    assert result == config_file
    content = config_file.read_text()
    assert '[general]\nmodel = "openai-chat:gpt-4o"' in content
    assert '[display]\ntheme = "cappuccino"\nmax_output_lines = 500' in content
    assert config_manager.reload().display.theme == "cappuccino"


def test_persist_display_theme_updates_project_config_when_active(
    config_manager: ConfigManager,
    temp_project_dir: Path,
) -> None:
    project_config_dir = temp_project_dir / ConfigManager.PROJECT_CONFIG_DIR
    project_config_dir.mkdir()
    config_file = project_config_dir / "config.toml"
    config_file.write_text('[display]\ntheme = "dark"\n')

    result = config_manager.persist_display_theme("light")

    assert result == config_file
    assert config_file.read_text() == '[display]\ntheme = "light"\n'

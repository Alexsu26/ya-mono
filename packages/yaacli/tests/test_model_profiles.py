...
def test_startup_profile_uses_legacy_active_model_without_general_default(tmp_path: Path) -> None:
    """Migrated fork configs retain their configured startup profile."""
    config = YaacliConfig.model_validate(
        {
            "general": {"active_model": "glm-5.2"},
            "models": {"glm-5.2": {"model": "zai@openai-chat:glm-5.2"}},
            "model_profiles": {"subs-codex": {"model": "oauth@codex:gpt-5.5"}},
        }
    )
    ...
    assert profile is not None
    assert profile.id == "glm-5.2"
    assert profile.model == "zai@openai-chat:glm-5.2"

...
if TYPE_CHECKING:
    ...
    from ya_agent_sdk.mcp import MCPConfig
...
def apply_model_profile(
    ...
    """Apply a resolved model profile to an idle runtime."""
    model_extra_headers = runtime.ctx.get_model_extra_headers() if profile.model.startswith("oauth@codex:") else None
    runtime.agent.model = infer_model(profile.model, extra_headers=model_extra_headers)
    runtime.agent.model_settings = cast(ModelSettings, resolve_model_settings(profile.model_settings))
    runtime.ctx.model_cfg = resolve_profile_model_cfg(profile.model_cfg)
